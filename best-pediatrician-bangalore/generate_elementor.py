...
For: Best Pediatrician in Bangalore
"""
...
# Read HTML
with open('best-pediatrician-bangalore.html', 'r', encoding='utf-8') as f:
    html_content = f.read()
...
# Extract script - more careful regex to avoid ld+json
scripts = re.findall(r'<script[^>]*>(?!.*ld\+json)(.*?)</script>', html_content, re.DOTALL)
...
TESTIMONIALS_BODY = extract_section('testimonials')
...
TESTIMONIALS_HTML = wrap_section(TESTIMONIALS_BODY, 'testimonials-section', 'testimonials')
...
<style id="best-bangalore-styles" data-scope="pixel-perfect">{CSS_WITH_ISOLATION}</style>"""

# Mobile CTA with embedded script
MOBILE_CTA_SCRIPTS = f"""<div class="mobile-sticky-cta" id="best-bangalore-mobile-cta">
...
template = {
    "version": "0.4",
    "title": "Best Pediatrician in Bangalore – Dr. Shivashankar Diggikar (Pixel-Perfect v2)",
    "type": "page",
    "content": [
        ...
        make_section("#ffffff", CONDITIONS_HTML, "0", "0"),
        make_section("#ffffff", LOCATION_HTML, "0", "0"),
        make_section("#ffffff", TESTIMONIALS_HTML, "0", "0"),
...
output_path = "elementor-best-pediatrician-bangalore.json"
