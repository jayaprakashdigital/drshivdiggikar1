#!/usr/bin/env python3
"""
Elementor Generator for Top Pediatrician in Whitefield
...
# Read Whitefield Pediatrician HTML
with open('whitefield-pediatrician.html', 'r', encoding='utf-8') as f:
    html_content = f.read()
...
script_match = re.search(r'<script>(.*?)</script>(?!.*<script[^>]*type)', html_content, re.DOTALL)
...
<style id="whitefield-styles">{CSS}</style>"""
...
template = {
    "version": "0.4",
    "title": "Top Pediatrician in Whitefield Bangalore – Dr. Shivashankar Diggikar",
    "type": "page",
    "content": [
...
output_path = "elementor-whitefield-pediatrician.json"
